...
public class ResolverServiceLayer extends ServiceLayerDecorator {
	...
	public Class<?> resolveDomainClass(Class<?> clazz) {
		...
		Class<?> aClass = dump.getXstream().getMapper().lookupMapperOfType(
				clazz).defaultImplementationOf(clazz);
		return aClass == null ? (Class<?>) die(null,
				"Could not resolve a domain type for client type %s", clazz
						.getCanonicalName()) : aClass;
	}

	@Override
	public Method resolveDomainMethod(Method requestContextMethod) {
		Class<?> declaringClass = requestContextMethod.getDeclaringClass();
		Class<?> searchIn = getTop().resolveServiceClass(
